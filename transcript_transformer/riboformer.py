...
def main():
    ...
    for i, fold in args.folds.items():
        ...
        callback_path = impresources.files(riboformer_models) / f"{args.transfer_checkpoint}"
        ...
        np.save(f"{args.out}_out_f{i}.npy", np.array([ids, preds, targets], dtype=object).T)
    out = np.vstack([np.load(f"{args.out}_out_f{i}.npy", allow_pickle=True) for i in args.folds.keys()])
